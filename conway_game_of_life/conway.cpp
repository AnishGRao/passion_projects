...
std::vector<std::vector<cell>> old_board(board_size,
                                         std::vector<cell>(board_size)),
    new_board(board_size, std::vector<cell>(board_size));
  ...
  void execute() {
    ...
    if (num_alive_neighbors < 2)
      new_board[row][col].alive = 0;
             ...
             (num_alive_neighbors == 2 && old_board[row][col].alive))
...
void print_game() {
  ...
  char temp[1];
  for (auto i = old_board.begin() + 1;
       i != old_board.begin() + old_board.size() - 1; i++) {
    if (grid) {
      printw("| ");
    }
    for (auto j = (*i).begin() + 1; j != i->begin() + old_board[0].size() - 1;
         j++) {
      temp[0] = j->img[0];
      if (j->img[0] == 'X') {
        attron(COLOR_PAIR(1));
